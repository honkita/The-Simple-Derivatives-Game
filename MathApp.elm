...
init = { time = 0
       ...
       , showAnswer = False
       , freeplay = False
       , derivativesHint = Lue13.DerivativesHint.init
       , instructions = Lue13.GameInstructions.init
       , colourBlind = False
       , dark = True
       , right = False
       , prev = Home
...
type Msg = Tick Float GetKeyState
         ...
         | GenAnswer
         | Hints
         | DerivativesHint Lue13.DerivativesHint.Msg
         | Instructions Lue13.GameInstructions.Msg
         | CSwap String
         | ChangeSettings
         | ToQuestion String
         | Restart String
 ...
 in
  case msg of
    ...
    Hints ->
      ({model | state = Hint
              , prev = model.state
              , derivativesHint = if model.dark == True && model.colourBlind == True then Lue13.DerivativesHint.porquenolosdos
                                  else if model.dark == True then Lue13.DerivativesHint.d
                                  else if model.colourBlind == True then Lue13.DerivativesHint.b
                                  else Lue13.DerivativesHint.init
    ...
    DerivativesHint d -> 
                let 
                  newModel = Lue13.DerivativesHint.update d model.derivativesHint
                  
                in
                  ( { model | derivativesHint = if newModel.state == Lue13.DerivativesHint.NoHint then Lue13.DerivativesHint.init
                                        else newModel
                            , state = if newModel.state == Lue13.DerivativesHint.NoHint then model.prev
                                      else Hint}
                  , Cmd.none)
    ...
    Instructions i -> 
                let 
                  newModel = Lue13.GameInstructions.update i model.instructions
                  
                in
                  ( { model | state = if newModel.state == Lue13.GameInstructions.Done then Question
                                      else if newModel.state == Lue13.GameInstructions.Back then Home
                                      else model.state}
                  , generate (List.length questions))
...
whichonept2 model = 
  if model.dark == True && model.colourBlind == True then Lue13.GameInstructions.porquenolosdos
  else if model.dark == True then Lue13.GameInstructions.d
  else if model.colourBlind == True then Lue13.GameInstructions.b
  else Lue13.GameInstructions.init 

whichonept3 model = 
  if model.dark == True && model.colourBlind == True then Lue13.GameInstructions.porquenolosdos1
  else if model.dark == True then Lue13.GameInstructions.d1
  else if model.colourBlind == True then Lue13.GameInstructions.b1
  else Lue13.GameInstructions.init1
...
myShapes model = 
    case model.state of
      ...
      Hint -> 
          ...
          0 -> 
              [(Lue13.DerivativesHint.myShapes model.derivativesHint
                      |> List.map
      ...
      Tutorial -> 
              [(Lue13.GameInstructions.myShapes model.instructions
                      |> List.map
